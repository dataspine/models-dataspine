...
def generate_experiment_fn(**experiment_args):
  ...
  def _experiment_fn(run_config, hparams):
    ...
    train_input = lambda: model.generate_input_fn(
        [os.path.join(os.environ['DATASPINE_INPUT_PATH'], train_file) for train_file in hparams.train_files],
        num_epochs=hparams.num_epochs,
        batch_size=hparams.train_batch_size,
    )
    # Don't shuffle evaluation data
    eval_input = lambda: model.generate_input_fn(
        [os.path.join(os.environ['DATASPINE_INPUT_PATH'], eval_file) for eval_file in hparams.eval_files],
...
#      help='GCS location to write checkpoints and export models',
      #required=True,
#      default=os.environ['DATASPINE_OUTPUT_PATH']
#  )
  parser.add_argument(
      '--verbosity',
      choices=[
  ...
  learn_runner.run(
      ...
      run_config=tf.contrib.learn.RunConfig(model_dir=os.path.join(os.environ['DATASPINE_OUTPUT_PATH'], 
                                                                   'dataspine_tfserving/0')),
# Note:  The following is throwing this error:
#    return model_dir and model_dir.startswith("gs://")
#TypeError: startswith first arg must be bytes or a tuple of bytes, not str
#      run_config=tf.contrib.learn.RunConfig(model_dir=saved_model_export_utils.get_timestamped_export_dir(os.environ['DATASPINE_OUTPUT_PATH'])),
